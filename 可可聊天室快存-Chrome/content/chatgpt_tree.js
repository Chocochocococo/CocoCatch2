...
body{margin:0;padding:24px 16px 64px;background:var(--bg);color:var(--fg);
  font:16px/1.7 -apple-system,"Segoe UI","Noto Sans TC","Microsoft JhengHei",sans-serif}
.wrap{max-width:860px;margin:0 auto}
header{border-bottom:2px solid var(--line);padding-bottom:16px}
h1{font-size:24px;margin:0 0 8px}
.meta{color:var(--muted);font-size:13px;margin:2px 0}
.msg{border:1px solid;border-radius:10px;padding:12px 16px;margin:16px 0}
...
.who{font-weight:700;font-size:13px;color:var(--muted);margin-bottom:6px}
.who .ts{font-weight:400;margin-left:8px;opacity:.8}
.body>:first-child{margin-top:0}.body>:last-child{margin-bottom:0}
.body pre{background:var(--code-bg);padding:12px;border-radius:8px;overflow-x:auto}
.body code{background:var(--code-bg);padding:2px 5px;border-radius:4px;font-size:.9em}
...
.branch{margin:14px 0}
.branch-bar{display:flex;align-items:center;gap:10px;flex-wrap:wrap;font-size:13px;
...
.nav-btn{font:inherit;font-size:16px;line-height:1;width:28px;height:26px;padding:0;
...
.expanded .pane::before{content:attr(data-label);display:block;font-size:13px;
  font-weight:700;color:var(--accent);margin:10px 0 2px;padding-left:10px;
...
${stats.branchPoints ? `<div class="toolbar"><label><input type="checkbox" id="expandAll"> 展開所有分支版本（不用切分頁）</label></div>` : ""}
<main>
${out.join("\n")}
</main>
</div>
${stats.branchPoints ? `<script>${HTML_SCRIPT}<\/script>` : ""}
</body>
</html>
`;
